...
import org.fentanylsolutions.fentlib.mixins.early.fml.FMLControlledNamespacedRegistryAccessor;
import org.fentanylsolutions.fentlib.mixins.early.fml.RegistryDelegate_DelegateAccessor;
import org.fentanylsolutions.fentlib.mixins.early.minecraft.RegistryNamespacedAccessor;
import org.fentanylsolutions.fentlib.reflect.fml.GameDataReflect;
...
public class CarbonNamespacedRegistry<I> extends RegistryNamespaced {
    ...
    private BiMap<String, I> persistentSubstitutions = HashBiMap.create();
    private BiMap<String, I> activeSubstitutions = HashBiMap.create();

    private final BitSet localAvailabilityMap;

    public CarbonNamespacedRegistry(String optionalDefault, int maxIdValue, int minIdValue, Class<I> type, char discriminator)
    {
        this.superType = type;
        ...
        localAvailabilityMap = new BitSet(maxId + 1);
    }
    ...
    void validateContent(int maxId, String type, BitSet availabilityMap, Set<Integer> blockedIds, FMLControlledNamespacedRegistry<Block> iBlockRegistry)
    {
        for (I obj : typeSafeIterable())
        {
            ...
            if (obj instanceof ItemBlock)
            {
                ...
                // verify id range
                if (id > GameDataReflect.getMAX_BLOCK_ID()) throw new IllegalStateException(String.format("ItemBlock %s uses the id %d outside the block id range", name, id));
            }
        }

    }

    public void set(FMLControlledNamespacedRegistry<I> registry)
    {
        if (this.superType != ((FMLControlledNamespacedRegistryAccessor)registry).getSuperType()) throw new IllegalArgumentException("incompatible registry");

        this.discriminator = ((FMLControlledNamespacedRegistryAccessor)registry).getDiscriminator();
        this.optionalDefaultName = ((FMLControlledNamespacedRegistryAccessor)registry).getOptionalDefaultName();
        this.maxId = ((FMLControlledNamespacedRegistryAccessor)registry).getMaxId();
        this.minId = ((FMLControlledNamespacedRegistryAccessor)registry).getMinId();
        this.aliases.clear();
        this.aliases.putAll(((FMLControlledNamespacedRegistryAccessor)registry).getAliases());
        ...
        this.activeSubstitutions.putAll(((FMLControlledNamespacedRegistryAccessor)registry).getActiveSubstitutions());
    }

    private void useSlot(int slotId) {
        localAvailabilityMap.set(slotId);
    }

    private int registerObj(I obj, String name)
    {
        int itemId = this.add(-1, name, obj, localAvailabilityMap);
    ...
    public void addObject(int id, String name, Object thing)
    {
        if (thing instanceof Block || thing instanceof Item) {
            GameDataReflect.invokeRegister(GameDataReflect.invokeGetMain(), thing, name, id);
        } else {
            registerObj((I) thing, name);
        }
    }
    ...
    Map<String,Integer> getEntriesNotIn(FMLControlledNamespacedRegistry<I> registry)
    {
        ...
        for (I thing : this.typeSafeIterable())
        {
            if (!((RegistryNamespacedAccessor)registry).getField_148758_b().containsKey(thing))
            {
                if (!((FMLControlledNamespacedRegistryAccessor)registry).getActiveSubstitutions().containsKey(getNameForObject(thing)))
                {
    ...
    public RegistryDelegate<I> getDelegate(I thing, Class<I> clazz) {
        return GameDataReflect.invokeBuildDelegate(thing, clazz);
    }
    ...
    void activateSubstitution(String nameToReplace)
    {
        if (getPersistentSubstitutions().containsKey(nameToReplace))
        {
            ...
            if (superType == Item.class) {
                ...
                RegistryDelegate.Delegate<Item> delegate = (RegistryDelegate.Delegate<Item>)((Item)original).delegate;
                ((RegistryDelegate_DelegateAccessor)(Object)delegate).invokeChangeReference(sub);

                ((RegistryDelegate_DelegateAccessor)sub.delegate).invokeSetName(nameToReplace);
            }
    ...
    private BiMap<String, I> getPersistentSubstitutions()
    {
        ...
        {
            if (superType.equals(Item.class) || superType.equals(Block.class)) {
                persistentSubstitutions = GameDataReflect.invokeGetPersistentSubstitutionMap(GameDataReflect.invokeGetMain(), superType);
            }
        }
    ...
    void resetSubstitutionDelegates()
    {
        for (I item: typeSafeIterable()) {
            RegistryDelegate.Delegate<Item> delegate = (RegistryDelegate.Delegate<Item>)((Item)item).delegate;
            ((RegistryDelegate_DelegateAccessor)(Object)delegate).invokeChangeReference((Item)item);
        }
    }
    ...
    public String getNameForObject(Object obj)
    {
        if (obj instanceof IWrapper) {
            for (Object wrapper : this.field_148758_b.keySet()) {
                if (((IWrapper)wrapper).matchesWrapper((IWrapper) obj)) {
                    return (String) this.field_148758_b.get(wrapper);
                }
            }
        } else {
            return super.getNameForObject(obj);
        }
        return null;
    }
}
