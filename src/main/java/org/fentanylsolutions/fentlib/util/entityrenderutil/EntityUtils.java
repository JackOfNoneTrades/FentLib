package org.fentanylsolutions.fentlib.util.entityrenderutil;
...
// @formatter:on
    ...
    public static void drawEntityOnScreenAtRotation(int posX, int posY, float scale, float xAngle, float yAngle,
    ...
    public static void drawEntityOnScreenFollowMouse(int posX, int posY, float scale, float mouseX, float mouseY,
        EntityLivingBase ent) {
        GL11.glDisable(GL11.GL_BLEND);
        GL11.glDepthMask(true);
        GL11.glEnable(GL11.GL_DEPTH_TEST);
    ...
    public static void drawEntityOnScreen(int posX, int posY, float scale, float yaw, float yawOffset, float pitch,
        EntityLivingBase ent) {
        GL11.glDisable(GL11.GL_BLEND);
        GL11.glDepthMask(true);
        GL11.glEnable(GL11.GL_DEPTH_TEST);
    ...
    public static float getEntityScale(EntityLivingBase ent, float baseScale, float targetHeight) {
        return (targetHeight / Math.max(Math.max(ent.width, ent.height), ent.yOffset /* yOffset2 */)) * baseScale;
    }
    ...
    public static EntityLivingBase getRandomLivingEntity(World world, List blacklist, int numberOfAttempts,
        ...
        if (!EntityLivingBase.class.isAssignableFrom(clazz)) {
            if (fallbackPlayerNames != null) {
                ...
                return new EntityOtherPlayerMP(
                    world,
                    Minecraft.getMinecraft()
                        .func_152347_ac()
                        .fillProfileProperties(new GameProfile(entry.getKey(), entry.getValue()), true));
            } else {
